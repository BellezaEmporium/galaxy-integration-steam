...
1. Close GOG Galaxy.
1. Download the `windows.zip` file of the latest version in [the Releases page](https://github.com/BellezaEmporium/galaxy-integration-steam/releases).
...
You will need Python 3.7, and at least Python 3.7.9. You will then need to set up your python virtual environment, and then have `pip` get all the dependencies the project needs in your virtual environment. Once you have that, you can start making changes. Some IDEs will do this for you, but here are explicit instructions for doing it on your own.

### Environment Setup (Windows)

- Python 3.7.9 is the latest available Python 3.7 release you can easily get on Windows. it is available here: [Python 3.7.9][Python379]. Please make sure you use the 32-bit version, even on 64-bit machines. If you have a package manager that can get python 3.7, you may also use that.
...
- Python 3.7.9 is available as a package at [Python 3.7.9][Python379]. However, if you have another version of python installed, it is highly recommended you get `pyenv` and install python 3.7 through there. It's likely to be a newer version of python 3.7.
  - The easiest way to get pyenv is through `Homebrew`. This can be installed from [Homebrew � The Missing Package Manager for macOS (or Linux)](https://brew.sh/)
 Then, from terminal, you can run `brew install pyenv`
 To actually get python 3.7.9, run `pyenv install 3.7.9`. You can also use a newer version of 3.7, like 3.7.16
 Finally, tell this project to use python 3.7.x for all python commands from this folder.
 `pyenv local 3.7.9` or whatever newer version you used.
- Set up the virtual environment. This is the same for pyenv and only python 3.7.9:
`python -m venv .venv`
...
- Download [Python 3.7.9][Python379].
FOR WINDOWS:
  If you have another version of python installed, make sure `install py` is checked.
  This makes it easier to select which version of python you are using and we need our virtual environment in 3.7.9.
  If using Windows, make sure you have enabled the setting that `adds Python to the PATH environmental variable`.
  These should be the default settings, but make sure anyway.
FOR MAC: run `Install Certificates.command`

- Create a new virtual env:
  - If you only have python 3.7.9
 `python -m venv .venv`
  - If you have multiple python versions installed and are on Windows (assumes you have `py` as well)
 `py -3.7 -m venv .venv`
...
These will only be applied to the `.venv` virtual environment you created earlier:
  `pip install -r requirements/install.txt`
- Install the plugin. It should work if you have deleted the original plugin, but will patch it if it is there.
  `inv install`
