...
You will need Python 3.13.x. On Windows, match the Galaxy runtime bitness (32-bit vs 64-bit). You will then need to set up your python virtual environment, and then have `pip` get all the dependencies the project needs in your virtual environment. Once you have that, you can start making changes. Some IDEs will do this for you, but here are explicit instructions for doing it on your own.

### Environment Setup (Windows)

- Python 3.13.x is available here: [Python 3.13][Python313]. Please make sure you match the Galaxy runtime bitness. If you have a package manager that can get Python 3.13, you may also use that.
- If you have another version of python installed, it is highly recommended you install `py` when you go through the installer. Our tools are designed to use `py` if it's available, but fallback to just `python`. When you have multiple versions of python installed, `python` may not refer to 3.13 and that would break the code.
- Create a new virtual env:
 	- If you have py, run `py -3.13 -m venv .venv`
 	- If you only have python 3.13.x, run `python -m venv .venv`
...
- Python 3.13.x is available as a package at [Python 3.13][Python313]. However, if you have another version of python installed, it is highly recommended you get `pyenv` and install Python 3.13 through there.
 ...
 To actually get Python 3.13, run `pyenv install 3.13.0` (or a newer 3.13.x). <br/>
 Finally, tell this project to use Python 3.13.x for all python commands from this folder. <br/>
 `pyenv local 3.13.0` or whatever newer version you used.
- Set up the virtual environment. This is the same for pyenv and only Python 3.13.x:<br/>
`python -m venv .venv`
...
`./Install Certificates.command` You may need to allow it through gatekeeper. we recommend viewing the script before allowing it if you are uncomfortable with executing our script. It is copied directly from the Python 3.13 pkg, we just remove the install certifi command (we already did that).
...
- Download [Python 3.13][Python313]. FOR WINDOWS:
  <br> If you have another version of python installed, make sure `install py` is checked.
  <br> This makes it easier to select which version of python you are using and we need our virtual environment in 3.13.x.
...
- Create a new virtual env:
 	- If you only have python 3.13.x<br>
 `python -m venv .venv`
...
```
@echo You must have installed python 3.13.x. If not, the rest of this won't work.
py.exe -3.13 -m venv .venv
@echo If the previous command did not work, you do not have py installed or py is not in your PATH.
@echo If you only have python 3.13.x, run the next command.
...
```
echo You must have installed python 3.13.x (macOS). If not, the rest of this won't work.
py -3.13 -m venv .venv
echo If the previous command did not work, you do not have py installed or py is not in your PATH.
echo If you only have python 3.13.x, run the next command.
echo If it worked, skip the next command.
...
If `inv install` throws a bunch of errors, make sure you have the proper python venv set up. Errors about deprecated `inspect` APIs (like `getargspec`) usually mean you created the wrong virtual environment. You can either delete the `.venv` folder and reinstall it, or create a new virtual environment with a different name and use that.

Make sure you use `py -3.13` when creating your venv. If you don't have `py`, get it. You can specify the full path to Python 3.13 instead if you want, but that's harder to do and harder to explain here.
...
This is a fork of <https://github.com/ABaumher/galaxy-integration-steam>, which is itself a fork of <https://github.com/FriendsOfGalaxy/galaxy-integration-steam>.
...
[Python313]: https://www.python.org/downloads/
